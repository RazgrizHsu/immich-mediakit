...
//------------------------------------------------------------------------
window.LivePhotoManager = {
	hoveredVideo: null,
	modalVideo: null,

	init() {
		this.setupCardHoverListeners()
	...
	setupModalControls() {
		...
		// Update progress bar for modal video
		setInterval(() => {
	...
	toggleModalPlayback() {
		const video = document.querySelector('.livephoto-video-modal')
...
// Initialize LivePhoto manager when DOM is ready
if (document.readyState === 'loading') {
	document.addEventListener('DOMContentLoaded', () => window.LivePhotoManager.init())
} else {
	window.LivePhotoManager.init()
}
